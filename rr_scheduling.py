...
def cycles_begin(xs, y):
    start = random.sample(range(len(xs)),1)[0]
    sublist = [xs[i % len(xs)] for i in range(start, start + y)]
    # start = (start + y) % len(xs)
    # print(f"start={start}, sublist = {sublist}")
    return sublist

def cycles(xs, y, prev): ## xs is the main list, y is the length of the sublist, prev is the prev sublist
    start = 0 if prev[-1]==xs[-1] else (xs.index(prev[-1])+1)
    sublist = [xs[i % len(xs)] for i in range(start, start + y)]
    start = (start + y) % len(xs)
    return sublist

def find_rr_action(eval_env, slot): ## maximal age difference
    ...
    # rr_age_dest = {tuple(x):(eval_env.dest_age[tuple(x)] - eval_env.UAV_age[x[0]]) for x in eval_env.tx_rx_pairs}

    if verbose:
        print(f"prev_action_DL={eval_env.prev_action_DL}, prev_action_UL={eval_env.prev_action_UL}")

    if eval_env.prev_action_UL == None and eval_env.prev_action_DL == None:  ## will run the first time
        if verbose:
            print(f"FIRST")
        # download_user_pairs = eval_env.tx_rx_pairs #[0:eval_env.RB_total_DL]
        # upload_users   = eval_env.tx_users #[0:eval_env.RB_total_UL]
        # eval_env.prev_action_DL = download_user_pairs
        # eval_env.prev_action_UL = upload_users
        
        download_user_pairs = cycles_begin(eval_env.tx_rx_pairs, eval_env.RB_total_DL) #[0:eval_env.RB_total_DL]
        upload_users   = cycles_begin(eval_env.tx_users, eval_env.RB_total_UL) #[0:eval_env.RB_total_UL]
        eval_env.prev_action_DL = download_user_pairs
        eval_env.prev_action_UL = upload_users
        ...
        if verbose:
            print(f"ELSE")
        download_user_pairs = cycles(eval_env.tx_rx_pairs, eval_env.RB_total_DL, eval_env.prev_action_DL)
        upload_users = cycles(eval_env.tx_users, eval_env.RB_total_UL, eval_env.prev_action_UL)
        # download_user_pairs = cycles(eval_env.tx_rx_pairs, len(eval_env.tx_rx_pairs), eval_env.prev_action_DL)
        # upload_users = cycles(eval_env.tx_users, len(eval_env.tx_rx_pairs), eval_env.prev_action_UL)
        eval_env.prev_action_DL = download_user_pairs
        eval_env.prev_action_UL = upload_users
    ...
    download_user_pairs_arr = download_user_pairs[:eval_env.RB_total_DL]    
    upload_users_arr = upload_users[:eval_env.RB_total_UL]    
 ...
 ############################################
    ...
    # actual_rr_action = None # this has to change
    ...
    #     #     time.sleep(10)
    #     if (list(eval_action[0]))==(upload_users_arr) and (list(eval_action[1]))==(download_user_pairs_arr):
    #         actual_rr_action = action

    #         if verbose:
    #             print(f"upload_users_arr={upload_users_arr}, upload_users={upload_users}, download_user_pairs_arr={download_user_pairs_arr}, download_user_pairs={download_user_pairs}, actual_rr_action = {actual_rr_action}")    
                
    #         break
        
    return upload_users_arr, download_user_pairs_arr     
   


def rr_new_scheduling(I, drones_coverage, folder_name, deployment, packet_upload_loss_thresh, packet_download_loss_thresh, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL, BS_location, user_locations, MAX_STEPS):  ## maximal age difference
    
    # print(f"\nRR new started for {I} users , coverage = {drones_coverage} with update_loss_thresh = {packet_update_loss_thresh}, sample_loss_thresh = {packet_sample_loss_thresh}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users}, RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL} and {deployment} deployment", flush = True)
    
    print(f"\nRR new started for {I} users , coverage = {drones_coverage} with packet_upload_loss_thresh = {packet_upload_loss_thresh}, packet_download_loss_thresh = {packet_download_loss_thresh}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users},  RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL}  and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    rr_UL_schedule = {} ## key:([slot 1st DL completed, gen time of the packet], [slot 2nd DL completed, gen time of the packet], [...], [...])
    for i in tx_users:
        rr_UL_schedule[i] = [] ## list corresponding to each key

    rr_DL_schedule = {} ## key:([slot 1st UL completed, gen time of the packet], [slot 2nd UL completed, gen time of the packet], [...], [...])
    for i in tx_rx_pairs:
        rr_DL_schedule[tuple(i)] = [] ## list corresponding to each key
    ...
    for ep in range(random_episodes): # how many times the random policy will be run, similar to episode
        if ep%log_interval_random ==0 and ep!=0:
            ...
            print(f"RR ep = {ep}, {time_gap} seconds from start, rate = {round((1/rate),2)} eps/sec, finish_time = {finish_time} ", flush = True)
        ep_reward = 0
        ep_peak_reward = 0
        ...
        if ep==0:
            print(f"\nRR scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
            
            print(f"\nRR scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} ", file = open(folder_name + "/results.txt", "a"), flush = True)
        ...
        for x in range(MAX_STEPS):           
            upload_users, download_user_pairs = find_rr_action(eval_env, eval_env.current_step)
                        ...
                        if eval_env.RB_pending_DL[tuple(i)] == 0: # means packet was fully downloaded in this slot
                            ...
                            if eval_env.curr_DL_gen[tuple(i)] == -1: ## current packet started DL when BS had nothing 
                                ...
                                if (random_episodes-ep)<100: ##means the last 100 episode                                    
                                    rr_DL_schedule[tuple(i)].append([ep, eval_env.current_step + delay_from_throughput*delay_include, eval_env.curr_DL_gen[tuple(i)]]) 
                        ...
                        if eval_env.RB_pending_UL[i] == 0: # means packet was fully uploaded in this slot
                            ...
                            if eval_env.curr_UL_gen[i] == -1: ## current packet started UL when device had nothing 
                                ...
                                if (random_episodes-ep)<100: ##means the last 100 episodes
                                    rr_UL_schedule[i].append([ep, eval_env.current_step + delay_from_throughput*delay_include, eval_env.curr_UL_gen[i]])
                                       # new packet done
            ...
            if eval_env.current_step==MAX_STEPS:
                ...
                # print("ind age = ", eval_env.dest_age.values(), ", sum age at dest = ", final_reward)
    ...
    pickle.dump(overall_ep_peak_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_overall_ep_peak_reward.pickle", "wb"))    
    pickle.dump(final_step_peak_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_final_step_peak_rewards.pickle", "wb"))

    pickle.dump(overall_ep_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_overall_ep_reward.pickle", "wb"))
    pickle.dump(final_step_UAV_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_final_step_UAV_rewards.pickle", "wb"))
    pickle.dump(final_step_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_final_step_rewards.pickle", "wb"))

    pickle.dump(age_dist_UAV, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_age_dist_UAV.pickle", "wb"))
    pickle.dump(age_dist_dest, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_age_dist_dest.pickle", "wb"))
    
    pickle.dump(eval_env.sample_time, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_sample_time.pickle", "wb"))
    
    pickle.dump(success_upload, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_success_upload.pickle", "wb"))
    pickle.dump(success_download, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_success_download.pickle", "wb"))
    pickle.dump(attempt_upload, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_attempt_upload.pickle", "wb"))
    pickle.dump(attempt_download, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_attempt_download.pickle", "wb"))
    
    pickle.dump(unutilized_RB_DL, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_unutilized_RB_DL.pickle", "wb"))
    pickle.dump(unutilized_RB_UL, open(folder_name + "/" + deployment + "/" + str(I) + "U_rr_unutilized_RB_UL.pickle", "wb"))
    
    pickle.dump(rr_UL_schedule, open(folder_name + "/" + deployment + "/" + "rr_UL_schedule_" + str(I) + "U.pickle", "wb"))    
    pickle.dump(rr_DL_schedule, open(folder_name + "/" + deployment + "/" + "rr_DL_schedule_" + str(I) + "U.pickle", "wb"))  
     
    
    print("\nRR scheduling ", deployment, " placement, ", I, " users. MEAN of final_step_rewards = ", np.mean(final_step_rewards), ". MEAN of overall_ep_reward = ", np.mean(overall_ep_reward), " MIN and MAX of overall_ep_reward = ", np.min(overall_ep_reward),", ", np.max(overall_ep_reward),  " ... MEAN of overall_ep_peak_reward = ", np.mean(overall_ep_peak_reward), " MIN and MAX of overall_ep_peak_reward = ", np.min(overall_ep_peak_reward),", ", np.max(overall_ep_peak_reward), ". Similarly for final_step_UAV_rewards - MEAN = ",{np.mean(final_step_UAV_rewards)}, ", MIN and MAX of final_step_UAV_rewards = ", np.min(final_step_UAV_rewards),", ", np.max(final_step_UAV_rewards)," end with final state of ", eval_env._state, " with shape ", np.shape(eval_env._state), ", min PDR_upload = ", {np.min(PDR_upload)}, ", max PDR_upload = ", {np.max(PDR_upload)}, ", min PDR_download = ", {np.min(PDR_download)}, ", max PDR_upload = ", {np.max(PDR_download)}, ", max_total_packet_lost_upload = ", np.max(total_packet_lost_upload), ", min_total_packet_lost_upload = ", np.min(total_packet_lost_upload), ", max_total_packet_lost_download = ", np.max(total_packet_lost_download), ", min_total_packet_lost_download = ", np.min(total_packet_lost_download))
    ...
    print(f"rr best_episodes_average = {sorted_best_episodes_average}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"rr sorted_best_episodes_peak = {sorted_best_episodes_peak}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n ------------------------------------------------", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
