...
def find_omad_greedy_action(eval_env, slot): ## maximal age difference
    ...
    if verbose:
        print(f"\n\nIMPTT - tx_users = {eval_env.tx_users}, tx_rx_pairs = {eval_env.tx_rx_pairs}, upload_users = {upload_users}, non_upload_users = {non_upload_users}, excluded_pairs = {excluded_pairs}, non_excluded_pairs = {non_excluded_pairs}") ## working
...
def omad_greedy_UL_scheduling(I, drones_coverage, folder_name, deployment, packet_upload_loss_thresh, packet_download_loss_thresh, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL, BS_location, user_locations, MAX_STEPS):  ## maximal age difference
    ...
    print(f"\nomad_greedy_UL_scheduling started for {I} users , coverage = {drones_coverage} with packet_upload_loss_thresh = {packet_upload_loss_thresh}, packet_download_loss_thresh = {packet_download_loss_thresh}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users},  RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL}  and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    for ep in range(random_episodes): # how many times the random policy will be run, similar to episode
        if ep%log_interval_random ==0 and ep!=0:
            ...
            print(f"omad ep = {ep}, {time_gap} seconds from start, rate = {round((1/rate),2)} eps/sec, finish_time = {finish_time} ", flush = True)
        ep_reward = 0
        ep_peak_reward = 0
        ...
        if ep==0:
            print(f"\noMAD greedy UL scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
            
            print(f"\noMAD greedy UL scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} ", file = open(folder_name + "/results.txt", "a"), flush = True)
        ...
        while eval_env.current_TTI < MAX_STEPS:
            ...
            upload_users, download_user_pairs = find_omad_greedy_action(eval_env, eval_env.current_TTI)


            if verbose:
                print(f"\n\ncurrent episode = {ep}\n")
                print(f"slot = {eval_env.current_TTI} for OMAD greedy UL begins, upload_users = {upload_users}, download_user_pairs = {download_user_pairs}, BS_age = {eval_env.UAV_age}, dest_age = {eval_env.dest_age}\n")                
                        ...
                        if eval_env.RB_pending_DL[tuple(i)] == 0: # means packet was fully downloaded in this slot
                            ...
                            if eval_env.curr_DL_gen[tuple(i)] == -1: ## current packet started DL when BS had nothing 
                                ...
                                if (random_episodes-ep)<100: ##means the last 100 episode                                    
                                    omad_DL_schedule[tuple(i)].append([ep, eval_env.current_TTI + delay_from_throughput*delay_include, eval_env.curr_DL_gen[tuple(i)]]) 
                        ...
                        if eval_env.RB_pending_UL[i] == 0: # means packet was fully uploaded in this slot
                            ...
                            if eval_env.curr_UL_gen[i] == -1: ## current packet started UL when device had nothing 
                                ...
                                if (random_episodes-ep)<100: ##means the last 100 episodes
                                    omad_UL_schedule[i].append([ep, eval_env.current_TTI + delay_from_throughput*delay_include, eval_env.curr_UL_gen[i]])
                          # new packet done
    ...
    print(f"omad_UL_schedule = {omad_UL_schedule}, omad_DL_schedule = {omad_DL_schedule}")

    pickle.dump(age_dist_UAV_slot_wise, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_age_dist_UAV_slot_wise.pickle", "wb"))
    pickle.dump(age_dist_dest_slot_wise, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_age_dist_dest_slot_wise.pickle", "wb"))
 
    pickle.dump(best_episodes_average, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_episodes_average.pickle", "wb"))
    pickle.dump(best_episodes_peak, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_best_episodes_peak.pickle", "wb"))
    
    pickle.dump(overall_ep_peak_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_overall_ep_peak_reward.pickle", "wb"))    
    pickle.dump(final_step_peak_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_final_step_peak_rewards.pickle", "wb"))
    
    pickle.dump(overall_ep_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_overall_ep_reward.pickle", "wb"))
    pickle.dump(final_step_UAV_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_final_step_UAV_rewards.pickle", "wb"))
    pickle.dump(final_step_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_final_step_rewards.pickle", "wb"))
    
    pickle.dump(age_dist_UAV, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_age_dist_UAV.pickle", "wb"))
    pickle.dump(age_dist_dest, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_age_dist_dest.pickle", "wb"))
    
    pickle.dump(eval_env.sample_time, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_sample_time.pickle", "wb"))
    
    pickle.dump(success_upload, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_success_upload.pickle", "wb"))
    pickle.dump(success_download, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_success_download.pickle", "wb"))
    pickle.dump(attempt_upload, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_attempt_upload.pickle", "wb"))
    pickle.dump(attempt_download, open(folder_name + "/" + deployment + "/" + str(I) + "U_omad_greedy_attempt_download.pickle", "wb"))
    ...
    print(f"omad_greedy_UL_scheduling best_episodes_average = {sorted_best_episodes_average}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"omad_greedy_UL_scheduling sorted_best_episodes_peak = {sorted_best_episodes_peak}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n ------------------------------------------------", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
