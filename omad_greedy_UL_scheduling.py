...
def omad_greedy_UL_scheduling(I, drones_coverage, folder_name, deployment, packet_upload_loss_thresh, packet_download_loss_thresh, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL, BS_location, user_locations, MAX_STEPS):  ## maximal age difference
    ...
    for ep in range(random_episodes): # how many times the random policy will be run, similar to episode
        ...
        for x in range(MAX_STEPS):
            ...
            upload_users, download_user_pairs = find_omad_greedy_action(eval_env, eval_env.current_step)


            if verbose:
                print(f"\n\ncurrent episode = {ep}\n")
                print(f"slot = {eval_env.current_step} for OMAD greedy UL begins, upload_users = {upload_users}, download_user_pairs = {download_user_pairs}, BS_age = {eval_env.UAV_age}, dest_age = {eval_env.dest_age}\n")                
