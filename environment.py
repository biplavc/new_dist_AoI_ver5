...
log_interval_random = 10 # log the results after this many episodes
...
class UAV_network():   # network of UAVs not just a single one
    ...
    def __init__(self, n_users, coverage, name, folder_name, packet_upload_loss_thresh, packet_download_loss_thresh, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL, BS_location, user_locations, MAX_STEPS): 
    ...
    def get_current_state(self): #  
        ...
        if verbose:
            print(f'\nself.state of {self.name} get_current_state() = {self.state} with shape = {np.shape(self.state)}\n') # debug
        return (self.state)
