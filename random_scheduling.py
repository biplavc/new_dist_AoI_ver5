...
def random_scheduling(I, drones_coverage, folder_name, deployment, packet_upload_loss_thresh, packet_download_loss_thresh, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL, BS_location, user_locations, MAX_STEPS):  ## random
    

    print(f"\nrandom started for {I} users , coverage = {drones_coverage} with packet_upload_loss_thresh = {packet_upload_loss_thresh}, packet_download_loss_thresh = {packet_download_loss_thresh}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users},  RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL}  and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)   
    ...
    random_UL_schedule = {} ## key:([slot 1st DL completed, gen time of the packet], [slot 2nd DL completed, gen time of the packet], [...], [...])
    for i in tx_users:
        random_UL_schedule[i] = [] ## list corresponding to each key

    random_DL_schedule = {} ## key:([slot 1st UL completed, gen time of the packet], [slot 2nd UL completed, gen time of the packet], [...], [...])
    for i in tx_rx_pairs:
        random_DL_schedule[tuple(i)] = [] ## list corresponding to each key
    ...
    for ep in range(random_episodes): # how many times the random policy will be run, similar to episode
        if ep%log_interval_random ==0 and ep!=0:
            ...
            print(f"random ep = {ep}, {time_gap} seconds from start, rate = {round((1/rate),2)} eps/sec, finish_time = {finish_time} ", flush = True)
        ...
        if ep==0:            
            print(f"\nrandom scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
            
            print(f"\nrandom scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}", file = open(folder_name + "/results.txt", "a"), flush = True)
        ...
        while eval_env.current_TTI < MAX_STEPS:
            ...
            else: # everyone can be supported
                upload_users = eval_env.user_list
                download_user_pairs = eval_env.tx_rx_pairs
            ...
            for i in download_user_pairs:
                ...
                if received_SNR_download < eval_env.packet_download_loss_thresh[tuple(i)]:
                    if verbose:
                        print(f"PACKET LOSS - received_SNR_download = {received_SNR_download}, packet_download_loss_thresh = {eval_env.packet_download_loss_thresh[tuple(i)]}")
            ...
            for i in upload_users:
                ...
                if received_SNR_upload < eval_env.packet_upload_loss_thresh[i]:
                    if verbose:
                        print(f"PACKET LOSS - received_SNR_upload = {received_SNR_upload}, packet_upload_loss_thresh = {eval_env.packet_upload_loss_thresh[i]}")
    ...
    pickle.dump(overall_ep_peak_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_random_overall_ep_peak_reward.pickle", "wb"))    
    pickle.dump(final_step_peak_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_random_final_step_peak_rewards.pickle", "wb"))
      
    pickle.dump(overall_ep_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_random_overall_ep_reward.pickle", "wb"))
    pickle.dump(final_step_UAV_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_random_final_step_UAV_rewards.pickle", "wb"))
    pickle.dump(final_step_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_random_final_step_rewards.pickle", "wb"))
    ...
    print(f" random best_episodes_average = {sorted_best_episodes_average}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
