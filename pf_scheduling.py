...
def find_pf_action(eval_env, average_rate_source, average_rate_BS, BS_location, user_locations): ## maximal age difference
    ...
    for i in average_rate_source:
        if getSNR(BS_location, user_locations[i]) < eval_env.packet_upload_loss_thresh[i]:
            ...
            individual_rate_source[i] = 1
    
    new_average_rate_source = {}
    for i in average_rate_source:
        if average_rate_source[i] !=0:
            new_average_rate_source[i] = individual_rate_source[i]/average_rate_source[i]
        else:
            new_average_rate_source[i] = 10_000
        
    sampleDict_copy = copy.deepcopy(new_average_rate_source)    
    

    while len(upload_users) < len(average_rate_source):
        ...
        remaining_capacity = len(average_rate_source) - len(upload_users)
        if remaining_capacity > 0:
            upload_users.extend(random.sample(listOfKeys, len(listOfKeys)))
        for items in listOfKeys:
            del sampleDict_copy[items]
            
    upload_users_actual = upload_users[:eval_env.RB_total_DL]
    
    ## select UL pairs end
    
    
    ## select DL pairs start
    ...
    for i in average_rate_BS:
        if getSNR(BS_location, user_locations[i[1]]) < eval_env.packet_download_loss_thresh[tuple(i)]:
            ...
            individual_rate_BS[i] = 1
    
    new_average_rate_BS = {}
    for i in average_rate_BS:
        new_average_rate_BS[i] = individual_rate_BS[i]/average_rate_BS[i]
        
    sampleDict_copy = copy.deepcopy(new_average_rate_BS)    
    
    while len(download_user_pairs) < len(average_rate_BS):
        ...
        remaining_capacity = len(average_rate_BS) - len(download_user_pairs)
        if remaining_capacity > 0:
            download_user_pairs.extend(random.sample(listOfKeys, len(listOfKeys)))
        for items in listOfKeys:
            del sampleDict_copy[items]
            
    download_user_pairs = download_user_pairs[:eval_env.RB_total_DL]
    ...
    if verbose:
        print(f"\nnew_average_rate_source = {new_average_rate_source}, new_average_rate_BS = {new_average_rate_BS}\n")
        
    return upload_users_actual, download_user_pairs_arr     


def pf_scheduling(I, drones_coverage, folder_name, deployment, packet_upload_loss_thresh, packet_download_loss_thresh, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL, BS_location, user_locations, MAX_STEPS):  ## maximal age difference
    ...
    print(f"\nPF started for {I} users , coverage = {drones_coverage} with packet_upload_loss_thresh = {packet_upload_loss_thresh}, packet_download_loss_thresh = {packet_download_loss_thresh}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users},  RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL}  and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    age_dist_UAV_slot_wise = {} ## stores the average age at each step
    age_dist_dest_slot_wise = {}
    
    for ii in range(1, MAX_STEPS+1):
        age_dist_UAV_slot_wise[ii] = []
        age_dist_dest_slot_wise[ii] = []
    ...
    average_rate_user_source = {}
    average_rate_user_BS     = {}
    
    for i in tx_users:
        average_rate_user_source[i] = 1
    for i in tx_rx_pairs:
        average_rate_user_BS[tuple(i)] = 1
    
    pf_UL_schedule_ = {} ## key:([slot 1st DL completed, gen time of the packet], [slot 2nd DL completed, gen time of the packet], [...], [...])
    for i in tx_users:
        pf_UL_schedule_[i] = [] ## list corresponding to each key

    pf_DL_schedule_ = {} ## key:([slot 1st UL completed, gen time of the packet], [slot 2nd UL completed, gen time of the packet], [...], [...])
    for i in tx_rx_pairs:
        pf_DL_schedule_[tuple(i)] = [] ## list corresponding to each key
    ...
    for ep in range(random_episodes): # how many times the random policy will be run, similar to episode
        ...
        if ep==0:
            print(f"\nPF scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
            
            print(f"\nPF UL scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} ", file = open(folder_name + "/results.txt", "a"), flush = True)
        ...
        for x in range(MAX_STEPS):
            ...
            upload_users, download_user_pairs = find_pf_action(eval_env, average_rate_user_source, average_rate_user_BS, BS_location, user_locations)
            
            if verbose:
                print(f"\n\ncurrent episode = {ep} slot = {eval_env.current_step} for PF\n")
                print(f"initial: average_rate_user_BS = {average_rate_user_BS}, average_rate_user_source = {average_rate_user_source}")
            
            for i in average_rate_user_source:
                if i in upload_users:
                    average_rate_user_source[i] = 0.5*average_rate_user_source[i] + 0.5
                else:
                    average_rate_user_source[i] = 0.5*average_rate_user_source[i]
            
            yy = []
            for ii in download_user_pairs:
                yy.append(tuple(ii))
                    
              
            # for i in download_user_pairs:
            #     print(f"i for download_user_pairs = {i}")
    
            
            # for i in average_rate_user_BS:
            #     print(f"HERE i = {i}")
            #     if i in yy:
            #         average_rate_user_BS[i] = 0.5*average_rate_user_BS[i] + 0.5
            #         if verbose:
            #             print(f"HERE")
            #     else:
            #         average_rate_user_BS[i] = 0.5*average_rate_user_BS[i]
            
            if verbose:
                print(f"\nfinal: upload_users = {upload_users}, download_user_pairs = {download_user_pairs}, BS_age = {eval_env.UAV_age}, dest_age = {eval_env.dest_age}\n average_rate_user_BS = {average_rate_user_BS}, average_rate_user_source = {average_rate_user_source}")                
                        ...
                        if eval_env.RB_pending_DL[tuple(i)] == 0: # means packet was fully downloaded in this slot
                            ...
                            if eval_env.curr_DL_gen[tuple(i)] == -1: ## current packet started DL when BS had nothing 
                                ...
                                if (random_episodes-ep)<100: ##means the last 100 episode                                    
                                    pf_DL_schedule_[tuple(i)].append([ep, eval_env.current_step + delay_from_throughput*delay_include, eval_env.curr_DL_gen[tuple(i)]]) 
                        ...
                        if eval_env.RB_pending_UL[i] == 0: # means packet was fully uploaded in this slot
                            ...
                            if eval_env.curr_UL_gen[i] == -1: ## current packet started UL when device had nothing 
                                ...
                                if (random_episodes-ep)<100: ##means the last 100 episodes
                                    pf_UL_schedule_[i].append([ep, eval_env.current_step + delay_from_throughput*delay_include, eval_env.curr_UL_gen[i]])                    
                            # new packet done
            ...
            age_dist_dest_slot_wise[eval_env.current_step].append(np.mean(list(eval_env.dest_age.values())))
            age_dist_UAV_slot_wise[eval_env.current_step].append(np.mean(list(eval_env.UAV_age.values())))
    ...
    pickle.dump(age_dist_dest_slot_wise, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_age_dist_dest_slot_wise.pickle", "wb"))
 
    pickle.dump(best_episodes_average, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_episodes_average.pickle", "wb"))
    pickle.dump(best_episodes_peak, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_best_episodes_peak.pickle", "wb"))
    
    pickle.dump(overall_ep_peak_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_overall_ep_peak_reward.pickle", "wb"))    
    pickle.dump(final_step_peak_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_final_step_peak_rewards.pickle", "wb"))
    
    pickle.dump(overall_ep_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_overall_ep_reward.pickle", "wb"))
    pickle.dump(final_step_UAV_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_final_step_UAV_rewards.pickle", "wb"))
    pickle.dump(final_step_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_final_step_rewards.pickle", "wb"))
    
    pickle.dump(age_dist_UAV, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_age_dist_UAV.pickle", "wb"))
    pickle.dump(age_dist_dest, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_age_dist_dest.pickle", "wb"))
    
    pickle.dump(eval_env.sample_time, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_sample_time.pickle", "wb"))
    
    pickle.dump(success_upload, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_success_upload.pickle", "wb"))
    pickle.dump(success_download, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_success_download.pickle", "wb"))
    pickle.dump(attempt_upload, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_attempt_upload.pickle", "wb"))
    pickle.dump(attempt_download, open(folder_name + "/" + deployment + "/" + str(I) + "U_pf_attempt_download.pickle", "wb"))
    ...
    pickle.dump(pf_UL_schedule_, open(folder_name + "/" + deployment + "/" + "pf_UL_schedule_" + str(I) + "U.pickle", "wb"))    
    pickle.dump(pf_DL_schedule_, open(folder_name + "/" + deployment + "/" + "pf_DL_schedule_" + str(I) + "U.pickle", "wb"))  
    ...
    print(f"pf best_episodes_average = {sorted_best_episodes_average}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"pf sorted_best_episodes_peak = {sorted_best_episodes_peak}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n ------------------------------------------------", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
