...
def find_greedy_action(eval_env, slot): ## maximal age difference
    ...
    greedy_age_dest = eval_env.dest_age 
    sampleDict_copy = copy.deepcopy(greedy_age_dest)
    
    greedy_user_pairs = []
    while len(greedy_user_pairs) < len(greedy_age_dest):
        ...
        remaining_capacity = len(greedy_age_dest) - len(greedy_user_pairs)
        if remaining_capacity > 0:
            greedy_user_pairs.extend(random.sample(listOfKeys, len(listOfKeys)))
        for items in listOfKeys:
            del sampleDict_copy[items]
            
    greedy_user_pairs_actual = greedy_user_pairs[:eval_env.RB_total_DL]
            
    ## random select DL pairs end
    
    download_user_pairs = greedy_user_pairs_actual
    ...
    if verbose:
        print(f"\nslot {slot} begins. greedy age is {greedy_age_dest}, age at dest is {eval_env.dest_age}, age at UAV is {eval_env.UAV_age}.")  
 ...
 ############################################
    ...
    #         if verbose:
    #             print(f"upload_users={upload_users}, download_user_pairs={download_user_pairs_arr}, actual_greedy_action = {actual_greedy_action}")    
...
def greedy_scheduling(I, drones_coverage, folder_name, deployment, packet_upload_loss_thresh, packet_download_loss_thresh, periodicity, adj_matrix, tx_rx_pairs, tx_users, RB_needed_UL, RB_needed_DL, BS_location, user_locations, MAX_STEPS):  
        
    # print(f"\ngreedy started for {I} users , coverage = {drones_coverage} with update_loss_thresh = {packet_upload_loss_thresh}, packet_download_loss_thresh = {packet_download_loss_thresh}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users}, RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL} and {deployment} deployment", flush = True)
    
    print(f"\ngreedy started for {I} users , coverage = {drones_coverage} with packet_upload_loss_thresh = {packet_upload_loss_thresh}, packet_download_loss_thresh = {packet_download_loss_thresh}, periodicity = {periodicity}, tx_rx_pairs = {tx_rx_pairs}, tx_users = {tx_users},  RB_needed_UL = {RB_needed_UL}, RB_needed_DL = {RB_needed_DL}  and {deployment} deployment", file = open(folder_name + "/results.txt", "a"), flush = True)
    ...
    # all_actions = [] # just saving all actions to see the distribution of the actions ## remove_action
    ...
    greedy_UL_schedule = {} ## key:([slot 1st DL completed, gen time of the packet], [slot 2nd DL completed, gen time of the packet], [...], [...])
    for i in tx_users:
        greedy_UL_schedule[i] = [] ## list corresponding to each key

    greedy_DL_schedule = {} ## key:([slot 1st UL completed, gen time of the packet], [slot 2nd UL completed, gen time of the packet], [...], [...])
    for i in tx_rx_pairs:
        greedy_DL_schedule[tuple(i)] = [] ## list corresponding to each key  
    ...
    for ep in range(random_episodes): # how many times the random policy will be run, similar to episode
        if ep%log_interval_random ==0 and ep!=0:
            ...
            print(f"greedy ep = {ep}, {time_gap} seconds from start, rate = {round((1/rate),2)} eps/sec, finish_time = {finish_time} ", flush = True)
        ep_reward = 0
        ep_peak_reward = 0
        ...
        if ep==0:
            print(f"\ngreedy scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} \n\n", file = open(folder_name + "/action_space.txt", "a"), flush = True)
            
            print(f"\ngreedy scheduling and {deployment} placement with {I} users, coverage is {eval_env.act_coverage}, RB_total_UL is {RB_total_UL}, RB_total_DL = {RB_total_DL}, action space size is {eval_env.action_size} ", file = open(folder_name + "/results.txt", "a"), flush = True)
        ...
        for x in range(MAX_STEPS):
            ...
            # selected_action = find_greedy_action(eval_env, eval_env.current_step)
            ...
            upload_users, download_user_pairs = find_greedy_action(eval_env, eval_env.current_step)
                        ...
                        if eval_env.RB_pending_DL[tuple(i)] == 0: # means packet was fully downloaded in this slot
                            ...
                            if eval_env.curr_DL_gen[tuple(i)] == -1: ## current packet started DL when BS had nothing 
                                ...
                                if (random_episodes-ep)<100: ##means the last 100 episodes
                                    greedy_DL_schedule[tuple(i)].append([ep, eval_env.current_step + delay_from_throughput*delay_include, eval_env.curr_DL_gen[tuple(i)]])
                    ...
                    if eval_env.RB_pending_UL[i]==0: ## this user has completed the prev packet in its prev attempt, switch to a new packet now. both sample and try to send the new packet. also for the first slot, this wil be true                  
                        ...
                        if eval_env.RB_pending_UL[i] == 0: # means packet was fully uploaded in this slot
                            ...
                            if eval_env.curr_UL_gen[i] == -1: ## current packet started UL when device had nothing 
                                ...
                                if (random_episodes-ep)<100: ##means the last 100 episodes
                                    greedy_UL_schedule[i].append([ep, eval_env.current_step + delay_from_throughput*delay_include, eval_env.curr_UL_gen[i]])
                            # new packet done
    ...
    # print(f"greedy_UL_schedule = {greedy_UL_schedule}, greedy_DL_schedule = {greedy_DL_schedule}")
    
    pickle.dump(best_episodes_average, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_episodes_average.pickle", "wb"))
    pickle.dump(best_episodes_peak, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_best_episodes_peak.pickle", "wb"))
    
    pickle.dump(overall_ep_peak_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_overall_ep_peak_reward.pickle", "wb"))    
    pickle.dump(final_step_peak_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_final_step_peak_rewards.pickle", "wb"))
    
    pickle.dump(overall_ep_reward, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_overall_ep_reward.pickle", "wb"))
    pickle.dump(final_step_UAV_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_final_step_UAV_rewards.pickle", "wb"))
    pickle.dump(final_step_rewards, open(folder_name + "/" + deployment + "/" + str(I) + "U_greedy_final_step_rewards.pickle", "wb"))
    ...
    print(f"greedy best_episodes_average = {sorted_best_episodes_average}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"greedy sorted_best_episodes_peak = {sorted_best_episodes_peak}", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
    print(f"\n\n\n ------------------------------------------------", file = open(folder_name + "/best_episodes.txt", "a"), flush = True)
